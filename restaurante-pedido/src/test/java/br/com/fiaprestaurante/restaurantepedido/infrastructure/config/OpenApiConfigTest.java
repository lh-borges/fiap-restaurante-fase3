...
class OpenApiConfigTest {
    ...
    void deveDocumentarEndpointGraphqlProtegidoComExemplos() {
        OpenAPI openApi = new OpenApiConfig().restaurantePedidoOpenApi();
        var operation = openApi.getPaths().get("/graphql").getPost();

        assertThat(openApi.getInfo().getTitle()).contains("Pedidos");
        ...
        assertThat(operation.getRequestBody().getContent().get("application/json").getExamples())
                .containsKeys("CriarPedido", "ConfirmarPedido", "PedidoPorId", "MeusPedidos", "StatusModulo");
