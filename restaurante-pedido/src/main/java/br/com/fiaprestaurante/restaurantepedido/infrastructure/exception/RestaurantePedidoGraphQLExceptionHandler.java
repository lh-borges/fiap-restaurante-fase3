package br.com.fiaprestaurante.restaurantepedido.infrastructure.exception;
...
/**
 * Tratamento global de excecoes para resolvers GraphQL do modulo restaurante-pedido.
 */
@ControllerAdvice
public class RestaurantePedidoGraphQLExceptionHandler {

    private static final Logger log = LoggerFactory.getLogger(RestaurantePedidoGraphQLExceptionHandler.class);

    @GraphQlExceptionHandler(PedidoNaoEncontradoException.class)
    public GraphQLError handlePedidoNaoEncontrado(PedidoNaoEncontradoException exception,
    ...
    public GraphQLError handleUnexpected(Exception exception, DataFetchingEnvironment environment) {
        log.error("Erro inesperado em resolver GraphQL de restaurante-pedido", exception);
