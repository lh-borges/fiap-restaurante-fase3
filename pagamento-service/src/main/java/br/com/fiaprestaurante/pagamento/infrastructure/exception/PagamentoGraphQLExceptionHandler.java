package br.com.fiaprestaurante.pagamento.infrastructure.exception;
...
/**
 * Tratamento global de excecoes para resolvers GraphQL do modulo de pagamento.
 *
 * <p>Sem este handler, erros de regra de negocio e argumentos invalidos podem
 * ser expostos como erro interno generico para o cliente GraphQL.
 */
@ControllerAdvice
public class PagamentoGraphQLExceptionHandler {

    private static final Logger log = LoggerFactory.getLogger(PagamentoGraphQLExceptionHandler.class);

    @GraphQlExceptionHandler(PagamentoNaoEncontradoException.class)
    public GraphQLError handlePagamentoNaoEncontrado(PagamentoNaoEncontradoException exception,
    ...
    public GraphQLError handleUnexpected(Exception exception, DataFetchingEnvironment environment) {
        log.error("Erro inesperado em resolver GraphQL de pagamento", exception);
